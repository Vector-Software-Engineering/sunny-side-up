...
export default function AnswerEntry({ entry }) {
  ...
  return (
    <>
      ...
      <div style={{fontSize: '14px', color: "grey"}}> by {entry.answerer_name} | {formatDate} | Helpful? <WordIncrement onClick={(e) => handleAnswerHelpful(e, entry.id)}><u>Yes</u></WordIncrement> ({clickHelpful ? entry.helpfulness + 1 : entry.helpfulness}) | <WordIncrement><u onClick={(e) => handleAnswerReport(e, entry.answer_id)}>{clickReport ? "Reported" : "Report"}</u></WordIncrement></div>
    </>
  )
}